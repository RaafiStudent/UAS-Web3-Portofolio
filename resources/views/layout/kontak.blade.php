<x-layout :title="$title ?? 'Hubungi Saya'">
    <section class="py-20 bg-[#0B0F19]">
        <div class="container mx-auto px-6 max-w-6xl">
            ...
            {{-- ALERT SUKSES --}}
            @if(session('success'))
                <div class="mb-8 p-4 bg-green-500/10 border border-green-500 text-green-400 rounded-lg text-center">
                    {{ session('success') }}
                </div>
            @endif
            ...
            <div class="grid md:grid-cols-2 gap-12 bg-[#0F131D] p-8 md:p-12 rounded-xl border border-gray-800 shadow-2xl">
                ...
                {{-- Contact Information (SAMA SEPERTI SEBELUMNYA) --}}
                <div>
                ...
                {{-- Form Kirim Pesan (YANG DIPERBAIKI) --}}
                <div>
                    ...
                    {{-- Action diarahkan ke route store, Method POST --}}
                    <form action="{{ route('contact.store') }}" method="POST" class="space-y-6">
                        @csrf {{-- Wajib ada untuk keamanan Laravel --}}
                        
                        <div>
                            {{-- Tambahkan name="name" --}}
                            <input type="text" name="name" placeholder="Nama Anda" class="w-full px-5 py-3 bg-[#141822] border border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-400 text-gray-100" required>
                        </div>
                        <div>
                            {{-- Tambahkan name="email" --}}
                            <input type="email" name="email" placeholder="Email Anda" class="w-full px-5 py-3 bg-[#141822] border border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-400 text-gray-100" required>
                        </div>
                        <div>
                            {{-- Tambahkan name="subject" --}}
                            <input type="text" name="subject" placeholder="Subjek" class="w-full px-5 py-3 bg-[#141822] border border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-400 text-gray-100" required>
                        </div>
                        <div>
                            {{-- Tambahkan name="message" --}}
                            <textarea name="message" placeholder="Pesan Anda" rows="6" class="w-full px-5 py-3 bg-[#141822] border border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-400 text-gray-100" required></textarea>
                        </div>
                        <div>
                            <button type="submit" class="bg-cyan-500 hover:bg-cyan-600 text-gray-900 w-full px-6 py-3 rounded-lg font-semibold flex items-center justify-center gap-2 transition-transform duration-300 hover:scale-[1.01] shadow-lg shadow-cyan-500/20">
